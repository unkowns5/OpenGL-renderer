...
bool loadOBJMTL(
    ...
    //! Do this for all child meshes - at the moment only the first mesh is processed
    ...
    //! make sure to load all materials - at the moment we load only one material - use scene->mNumMaterials to access all materials\
    //! make sure to get all material parameters, at the moment we only set the diffuse color. Add ambient, specular, transparency as well\
    //! use scene->mMaterials[i]->GetTexture(aiTextureType_DIFFUSE,0, &texpath, NULL,NULL, NULL,NULL,NULL) == AI_SUCCESS) to access texture path
    if(scene->mNumMaterials >0){
		for (unsigned int i = 0; i < scene->mNumMaterials; i++) {
			...
			if (scene->mMaterials[i] != NULL) {
				...
				scene->mMaterials[i]->Get(AI_MATKEY_OPACITY, opacityV);
				newMat->setOpacity(opacityV);
				scene->mMaterials[i]->Get(AI_MATKEY_SHININESS, shiny);
				newMat->setShininess(shiny);
				if (scene->mMaterials[i]->GetTexture(aiTextureType_DIFFUSE, 0, &texpath, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS) {
					string sTexturePath =texpath.data;
					Texture* texture = new Texture(sTexturePath);
					newMat->setTextureName(sTexturePath.c_str());
				}
